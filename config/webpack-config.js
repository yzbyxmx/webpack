...
function getAst (filename) {
    ...
    if(config.module){
        config.module.rules.length && config.module.rules.map((cur,index) => {
            if(cur.test.test(config.entry)) {
                if(cur.use.length){
                    for(let i = cur.use.length -1 ;i>=0;i--){
                        var reverse = require(cur.use[i].loader ? cur.use[i].loader : cur.use[i])
...
function parse(fileName, entry) {
    let filePath = fileName.indexOf('.') === -1 ? fileName + '.js' : fileName
    //let filePath = fileName.indexOf('.js') === -1 ? fileName  : fileName
...
function bundleFile(option) {
    ...
    //console.log(90,config.module.rules)
   // return
